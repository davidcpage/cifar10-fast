...
def main():
    ...
    model = Network(net(), loss=x_ent_loss).to(device).half()
    ...
    for size in [batch_size, len(dataset['valid']['targets']) % batch_size]:
        warmup_cudnn(model, random_batch(size))
    ...
    test_set = list(zip(transpose(normalise(dataset['test']['data'], mean, std), 'NHWC', 'NCHW'), dataset['valid']['targets']))
    print(f'Finished in {timer():.2} seconds')
    ...
    logs, state, timer = (Table(), TSV), {MODEL: model, OPTS: [SGD(trainable_params(model).values(), opt_params)]}, Timer(torch.cuda.synchronize)
